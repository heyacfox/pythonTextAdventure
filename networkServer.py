...
class Server:
    ...
    def run(self):
        ...
        while running:
            ...
            self.idToAddress[str(c.address)] = str(c.address)
            self.addressToId[str(c.address)] = str(c.address)
    ...
    def run(self):
        ...
        while running:
            ...
            if data:
                ...
                self.sendData(str(addressToId[self.address]), data.decode('UTF-8'))
            else:
                self.client.close()
                running = 0
                #ELSE WE JUST DIE WHY???
                
    def sendData(clientId, dataAsString):
